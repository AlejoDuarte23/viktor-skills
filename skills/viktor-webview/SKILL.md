---
name: viktor-webview
description: Use when the user wants a VIKTOR app to render custom HTML, CSS, JavaScript, interactive dashboards, canvas tools, Plotly or browser-library visualizations, MathJax equation reports, or WebView interactions that send values back to the VIKTOR parametrization with `WebView`, `WebResult`, `WebAndDataView`, or the VIKTOR JavaScript SDK.
---
...
7. Load `../viktor-webview-mathjax/SKILL.md` when the HTML template must render LaTeX or MathJax equations.
8. Treat all dynamic HTML values as untrusted. Encode data with `json.dumps`, escape text, avoid interpolating user-provided HTML, and be deliberate about CDN dependencies.
9. Test decorated `WebView` and `WebAndDataView` methods with `viktor.testing.mock_View`.
...
- Load `../viktor-webview-mathjax/SKILL.md` for sandbox-safe MathJax equation templates.
