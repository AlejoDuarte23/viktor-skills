...
description: Use when the user wants a VIKTOR app to run ETABS through a VIKTOR worker on a Windows machine or server. VIKTOR cloud apps cannot directly automate local CSI desktop software, so this skill covers `vkt.etabs.ETABSAnalysis`, the inherited `PythonAnalysis` worker pattern, ETABS local execution assumptions, JSON input and output files, CSI API scripts, result extraction, error handling, requirements, and testing.
---
...
Do not use this skill for STAAD.Pro. Use `../viktor-staad-pro-worker/SKILL.md` for STAAD/OpenSTAAD work.
...
8. In the worker script, read files from `Path.cwd()`, start ETABS with the CSI COM API, build or update the model, run the analysis, extract results, and write JSON output.
